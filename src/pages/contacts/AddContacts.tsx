...
function AddContacts() {
  ...
  const submitFormWithDescription = (description: string) => {
    const Header = {
    ...
    fetchData(`${ContactUrl}/`, "POST", JSON.stringify(data), Header)
      .then((res: any) => {
        console.log("Response2:", res); // add this

        if (!res.error) {
          setMsg(res.message || "Contact created successfully");
          setError(false);
          resetForm();
          alert(res.message || "Contact created successfully");
          navigate("/app/contacts");
        } else {
          setError(true);
          console.log("Detailed errors:", res?.errors); // <-- add this
